...
```
openssl req -x509 -newkey rsa:2048 -keyout certs/localhost.dev.key -out certs/localhost.dev.crt -days 365 -nodes
```
3. Spustiť Built-in PHP web server na `localhost.dev:3000`
```
php -S localhost.dev:3000 -t .
```
4. Spustiť socat na porte 3001
```
sudo socat openssl-listen:3001,reuseaddr,cert=certs/localhost.dev.crt,key=certs/localhost.dev.key,verify=0,fork tcp-connect:localhost.dev:3000
```
Aplikácia beží na https://localhost.dev:3001 so self-signed SSL certifikátom.
